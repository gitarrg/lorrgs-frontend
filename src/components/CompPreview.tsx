...
function create_icon(prefix: string, name: string, count: string, op: string) {
    ...
    const icon_path = `${ASSETS}/images/${prefix}/${name}.webp`;
    ...
    return (
        <div
            key={name}
            className={`${styles.icon} rounded border-mid wow-${class_name} wow-border-${class_name} ${
                excluded ? "excluded" : ""
            }`}
        >
...
export default function CompPreview({
    ...
    return <div className={styles.comp_preview}>{icons.length > 0 ? icons : <h1>{placeholder}</h1>}</div>;
}
